...
create table flotacoches.coche (
	...
	numero_poliza int4 NULL,
	km_totales int4 NULL,
	modelo VARCHAR(50) null,
	aseguradora VARCHAR(50) null,
	fecha_alta_seguro date null,
	fecha_revision date null,
	km_revision int4 null,
	importe_revision float4 null,
	grupo VARCHAR(50) null,
	marca VARCHAR(50) null,
	primary key (matricula, fecha_revision)
);
	
create table flotacoches.grupo_empresarial (
	grupo varchar(50), 
	marca varchar(50), 
	modelo varchar(50),
...
--RELACIONES
alter table flotacoches.revision add constraint pk_revision_matricula_fecha foreign key (matricula, fecha_revision) references flotacoches.coche(matricula, fecha_revision);--hecho
alter table flotacoches.coche add constraint pk_grupo_marca_modelo foreign key (marca, modelo) references flotacoches.grupo_empresarial(marca, modelo);
...
insert  into flotacoches.grupo_empresarial (grupo, marca, modelo)
select distinct grupo, marca, modelo
from coches;

insert  into 
flotacoches.coche (matricula, fecha_compra, color, moneda, numero_poliza, km_totales, modelo, aseguradora, fecha_alta_seguro, fecha_revision, km_revision, importe_revision, marca, grupo)
select distinct matricula, fecha_compra, color, moneda, n_poliza, kms_totales, modelo, aseguradora, fecha_alta_seguro, fecha_revision, kms_revision, importe_revision, marca, grupo
from coches c;

insert into flotacoches.revision (fecha_revision, km_revision, importe_revision, matricula) 
select c.fecha_revision, c.kms_revision, c.importe_revision, c.matricula 
from coches c;

--HACEMOS LAS CONSULTAS

select * from flotacoches.grupo_empresarial order by  modelo, marca, grupo;

select distinct color, matricula, km_totales, aseguradora, numero_poliza, fecha_compra
from flotacoches.coche c 
order by color, matricula, km_totales, aseguradora, numero_poliza, fecha_compra; 

select distinct c.matricula, r.km_revision, r.fecha_revision, r.importe_revision
from flotacoches.coche c
join flotacoches.revision r on c.matricula = r.matricula
order by c.matricula, r.km_revision, r.fecha_revision, r.importe_revision;

select distinct ge.modelo, ge.marca, ge.grupo, c.fecha_compra, c.matricula, c.color, c.km_totales, c.aseguradora, c.numero_poliza
from flotacoches.coche as c
join flotacoches.grupo_empresarial as ge on c.marca = ge.marca and c.modelo = ge.modelo
order by fecha_compra desc, numero_poliza desc;
