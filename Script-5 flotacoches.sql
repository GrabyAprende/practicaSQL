create schema flotacoches;

--CREAMOS TABLAS
create table flotacoches.coche (
	matricula varchar(50) primary key,
	km_totales int4 not NULL,
	fecha_compra date not null,
	id_modelo int not null,
	id_color int not null
);--bien
	
create table flotacoches.color (
	id SERIAL primary key,
	color varchar(50) not null
);
	
create table flotacoches.moneda (
	id SERIAL primary key,
	moneda varchar(50) not null
);--bien
	
create table flotacoches.poliza (
	id SERIAL primary key,
...
create table flotacoches.aseguradora (
	id SERIAL primary key,
...
create table flotacoches.revision (
	id SERIAL primary key,
	fecha_revision date not null,
	km_revision int4 not null,
...
--RELACIONES
alter table flotacoches.revision add constraint fk_revision_coche_matricula foreign key (matricula) references flotacoches.coche(matricula);
alter table flotacoches.revision add constraint fk_revision_moneda foreign key (id_moneda) references flotacoches.moneda(id);
alter table flotacoches.poliza add constraint fk_poliza_coche foreign key (matricula) references flotacoches.coche(matricula);
alter table flotacoches.poliza add constraint fk_poliza_aseguradora foreign key (id_aseguradora) references flotacoches.aseguradora(id);
alter table flotacoches.coche add constraint fk_coche_color foreign key (id_color) references flotacoches.color(id);
alter table flotacoches.coche add constraint fk_coche_modelo foreign key (id_modelo) references flotacoches.modelo(id);
alter table flotacoches.modelo add constraint fk_modelo_marca foreign key (id_marca) references flotacoches. marca(id);
alter table flotacoches.marca add constraint fk_marca_grupo foreign key (id_grupo) references flotacoches.grupo(id);
...
insert  into flotacoches.grupo (grupo)
select  grupo
from coches group by grupo;

insert into flotacoches.marca (id_grupo, marca)
select g.id, c.marca
from coches c
join flotacoches.grupo g on g.grupo = c.grupo
group  by g.id, c.marca;

insert into flotacoches.modelo (id_marca, modelo) 
select ma.id, c.modelo  
from coches c
join flotacoches.grupo g on g.grupo = c.grupo
join flotacoches.marca ma on ma.marca = c.marca
group by g.id, ma.id, c.modelo;

insert  into flotacoches.aseguradora (aseguradora)
select  aseguradora
from coches c group by aseguradora;

insert  into flotacoches.moneda (moneda)
select  moneda
from coches c group by moneda;

insert  into flotacoches.color (color)
select  color
from coches c group by color;

insert into flotacoches.coche (matricula, km_totales, fecha_compra, id_modelo, id_color) 
select matricula, kms_totales, fecha_compra, mo.id, co.id  
from coches c
join flotacoches.grupo g on g.grupo = c.grupo
join flotacoches.marca ma on ma.marca = c.marca
join flotacoches.modelo mo on mo.modelo = c.modelo
join flotacoches.color co on co.color = c.color
group by matricula, kms_totales, fecha_compra, mo.id, co.id;


insert into flotacoches.poliza (matricula, id, id_aseguradora, fecha_alta_seguro, activo) 
select matricula, n_poliza, a.id, fecha_alta_seguro, false  
from coches c
join flotacoches.aseguradora a on a.aseguradora = c.aseguradora
group by matricula, n_poliza, a.id, fecha_alta_seguro
order by matricula, n_poliza;

update flotacoches.poliza as p 
set activo = true 
from (select p1.matricula, max(p1.fecha_alta_seguro) as max_fecha_alta from flotacoches.poliza as p1
group by p1.matricula) as pa 
where p.matricula = pa.matricula and p.fecha_alta_seguro = pa.max_fecha_alta;

insert into flotacoches.revision (fecha_revision, km_revision, importe_revision, id_moneda, matricula) 
select fecha_revision, kms_revision, importe_revision, m.id, matricula  
from coches c
join flotacoches.moneda m on m.moneda = c.moneda
group by fecha_revision, kms_revision, importe_revision, m.id, matricula
order by c.matricula;



select g.grupo, mo.modelo, ma.marca, c.fecha_compra, c.matricula, co.color, c.km_totales, a.aseguradora, p.id poliza
from flotacoches.coche c 
inner join flotacoches.modelo mo on mo.id = c.id_modelo 
inner join flotacoches.marca ma on ma.id = mo.id_marca
inner join flotacoches.grupo g on g.id = ma.id_grupo
inner join flotacoches.poliza p on p.matricula  = c.matricula and p.activo = true 
inner join flotacoches.aseguradora a on a.id = p.id_aseguradora 
inner join flotacoches.color co on co.id = c.id_color;